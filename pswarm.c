...
extern int mve_presolve(int m, int n, double *A, double *b, int maxiter, double tol, double *x);
extern int mve_solver(int m, int n, double *A, double *b, double *x0, double maxiter, double tol, double *x, double *E2);

void check_feasible_pop(int n, int s, int lincons, struct swarm *pop, double *lb, double *ub,
						double *A, double *b);
#endif


int feasible_p(int n, double *x, int lincons, double *A, double *b, double *lb, double *ub);
void print_pop(int, int, int , struct swarm *);
int init_pop(int, int, int, struct swarm *, double *, double *, double *, double *, int, double *);
...
void scaledobjf(void (*objf)(), int, int, double *, double *);

extern void pollstep(int n, int lincons, int pi, void (*objf)(), double *lb, double *ub,
...
struct Options opt = { 
	...
	10,      /* bound limit */
	1.0e-1,  /* tolerance for stopping criteria -- high, since we are working with degrees */
...
int PSwarm(int n, void (*objf)(), double *llb, double *uub, int lincons, double *AA,
		   double *b, double **sol, double *f, double *xx)
{
	...
	ub   =malloc(n*sizeof(double));
	x    =malloc(n*sizeof(double));
	A    =malloc(n*lincons*sizeof(double));
	if(!scale || !lb || !ub || !A || !x){
		printf("Unable to allocate memory for scale vectors\n");
		exit(1);
	}
	...
	minublb=uub[0]-llb[0];
	for(i=1;i<n;i++){
		if(minublb>uub[i]-llb[i])
			minublb=uub[i]-llb[i];
	}

	if(minublb<1.0 || minublb>Inf)
		minublb=1.0;

	for(i=0;i<n;i++){
		scale[i]=(uub[i]-llb[i])/minublb;
		if(scale[i]>Inf)
			scale[i]=1.0;
		ub[i]=uub[i]/scale[i];
		lb[i]=llb[i]/scale[i];
		if(xx)
			x[i]=xx[i]/scale[i];
		for(j=0;j<lincons;j++)
			A[j+lincons*i]=AA[j+lincons*i]*scale[i];
	}
	...
	print_array(n*lincons, AA);
	print_array(n*lincons, A);
	print_array(lincons,b);*/
	...
	if(lincons>0 && opt.IPrint>=0){
		printf("\n**** Linear constraints defined but PSwarm was compiled without linear constraints support\n");
...
/*	if(!pop.x || !pop.v || !pop.y || !pop.fx || !pop.fy || !pop.active || ! maxv)
	...
	for(i=0;i<lincons;i++)
		if(pop.scale<fabs(b[i]))
			pop.scale=fabs(b[i]);
#endif

	if(opt.IPrint>=0)
		printf("Population scale: %f\n", pop.scale);
	...
	if(x){
		...
		//printf("Initial Population checked\n");
#endif
...
if(opt.vectorized){ /* call objf once with all the points */
	...
	for(j=0,i=0;i<opt.s;i++)
		if(pop.active[i] && feasible_p(n, &pop.x[i*n], lincons, A, b, lb, ub)){
			memcpy(&vectorx[j*n],&pop.x[i*n],n*sizeof(double));
			j++;
		}
		
	scaledobjf(objf, n, j, vectorx, vectorfx);
	...
	free(vectorfx);
			...
			for(i=0;i<opt.s;i++){
				if(pop.active[i]){
					...
					if(feasible_p(n, &pop.x[i*n], lincons, A, b, lb, ub)){
						scaledobjf(objf, n, 1, &pop.x[i*n], &pop.fx[i]);
						stats.objfunctions++;
					} else {
						pop.fx[i]=+Inf;
					}
			...
			for(i=0;i<opt.s;i++){ /* for each particle */
				...
				if(pop.active[i]){ /* active particle */
					...
					for(j=0;j<n;j++)
						velocity[j]=AlphaMax[j]*pop.v[i*n+j];

					if(lincons){
						/* -Ax */
						dgemv_(&Normal, &lincons, &n, &none, A, &lincons, &pop.x[i*n], &oneI, &zero, tmpm1, &oneI);

						/* Av */
						dgemv_(&Normal, &lincons, &n, &one, A, &lincons, velocity, &oneI, &zero, tmpm2, &oneI);

						for(j=0;j<lincons;j++){
							if(tmpm2[j]>0 && ((b[j]+tmpm1[j])/tmpm2[j])<AlphaMaxLinCons)
					...
					if(AlphaMaxLinCons>0.0){
						...
						for(j=0;j<n;j++){
							...
							pop.x[i*n+j]=projection(pop.x[i*n+j]+AlphaMaxLinCons*velocity[j],lb[j],ub[j]);
						}
					...
					/* update particle and check bound limits */
					for(j=0;j<n;j++){
						pop.x[i*n+j]=projection(pop.x[i*n+j]+AlphaMax[j]*pop.v[i*n+j],lb[j],ub[j]);
					}
#endif
				}
			}
			...
			for(i=0;i<opt.s;i++){ /* for each particle */
				if(pop.active[i] && i!=gbest){ /* active particle and not the gbest */
					...
					if(normtmp<sqrt(opt.delta) && normtmp2<sqrt(opt.delta)){ //(fabs((double)(iter-(opt.maxiter)/100.0)))){
						pop.active[i]--; /* remove particle */
						//printf("Particle %d inactive iter=%d normtmp=%f normtmp2=%f optdelta=%f\n", i, iter, normtmp, normtmp2, opt.delta);
...
int init_pop(int n, int s, int lincons, struct swarm *pop, double *lb, double *ub,
			 double *A, double *b, int ninitials, double *initials)
{
	int i, j;
	...
	if(ninitials && initials){
		for(i=0;i<ninitials;i++){
			...
			for(j=0;j<n;j++)
				pop->x[accepted*n+j]=projection(initials[i*n+j],lb[j],ub[j]);
			feas=1;
			...
			if(lincons>0){
				...
				dgemv_(&Normal, &lincons, &n, &one, A, &lincons, &initials[i*n], &oneI, &zero, Ai, &oneI);
	...
	if(lincons>0){
		...
		for(i=0;i<n;i++){
			if(ub[i]<Inf){
				Aext[lincons+tmplincons+i*reallincons]=1.0; /* remaining were set to zero */
		...
		if(msg){
			...
			if(reallincons>=lincons+2*n){
				printf("MVE cannot recover. All bounds already considered!\nAborting\n");
			...
			for(i=0;i<n;i++){
				if(ub[i]>=Inf){
					Aextext[reallincons+tmplincons+i*(lincons+2*n)]=1.0; /* remaining were set to zero */
					if(lb[i]>-Inf){
		...
		msg=mve_solver(reallincons, n, Aext, bext, center_ini, 80, 1e-6, center, E2);
		if(msg){
			...
			msg=mve_solver(tmplincons, n, Aextext, bext, center_ini, 80, 1e-6, center, E2);
			if(msg){
...
void scaledobjf(void (*objf)(), int n, int m, double *vectorx, double *vectorfx)
{
	double *unscaledx;
	int i,j;

	unscaledx= malloc(n*m*sizeof(double));
	if(!unscaledx){
		printf("Unable to allocate memory for unscaled points\n");
		printf("A big mess is going to happen\n");
		for(j=0;j<m;j++)
			vectorfx[j]=Inf;
		return;
	}
	...
	for(i=0;i<n;i++)
		for(j=0;j<m;j++)
			unscaledx[j*n+i]=scale[i]*vectorx[j*n+i];
	
	objf(n, m, unscaledx, vectorfx);
...
void print_best(int n, int gbest, int s, struct swarm *pop, int iter)
{
	...
	fprintf(file,"%d %f; ",stats.objfunctions,pop->fy[gbest]);
						...
						double *A, double *b)
{
	...
	for(pi=0;pi<s;pi++){
		feasible=1;
		/* check for linear feasibility */
		dgemv_(&Normal, &lincons, &n, &none, A, &lincons, &pop->x[pi*n], &oneI, &zero, tmpm, &oneI); /* tmpm = -Ax */
		for(i=0;i<lincons;i++)
			if(b[i]+tmpm[i]<-1e-5){
				feasible=0;
				how_many=b[i]+tmpm[i];
			...
			if(!feasible){
				printf("Particle %d is not linear feasible %.15f\nThis should not happen!!!\n", pi, how_many);
			}
			...
			if(!feasible){
				printf("Particle %d is not simple bound feasible\nThis should not happen!!!\n", pi);
			}
...
int feasible_p(int n, double *x, int lincons, double *A, double *b, double *lb, double *ub)
{
	...
	/* check for linear feasibility */
	if(lincons && A && b){
		dgemv_(&Normal, &lincons, &n, &none, A, &lincons, x, &oneI, &zero, tmpm1, &oneI); /* tmpm = -Ap */
		for(j=0;j<lincons;j++)
			if(b[j]+tmpm1[j]<-1e-5){
#ifndef linux
				free(tmpm1);
#endif
				//printf("Particle is not linear feasible. This should not happen!!!\n");
				return 0;
			}
	}
#ifndef linux
	...
	for(j=0;j<n;j++)
		if(x[j]>ub[j] || x[j]<lb[j]){
			//printf("Particle is not bound feasible. This should not happen!!!\n");
			return 0;
		}
