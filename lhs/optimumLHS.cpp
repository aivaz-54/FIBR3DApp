 ...
 *
 */
    void optimumLHS(int n, int k, int maxSweeps, double eps, bclib::matrixlhs<int> & outlhs,
                    int optimalityRecordLength, bclib::CRandom<double> & oRandom, bool bVerbose)
    {
        ...
        {
            outlhs = bclib::matrixlhs<int>(nsamples, nparameters);
        }
        //matrix_unsafe<int> oldHypercube_new = matrix_unsafe<int>(nsamples, nparameters, oldHypercube, true);
        bclib::matrixlhs<int> newHypercube = bclib::matrixlhs<int>(nsamples, nparameters);
