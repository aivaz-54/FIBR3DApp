...
namespace lhslib
{
     ...
     */
    void improvedLHS(int n, int k, int dup, bclib::matrixlhs<int> & result,
     ...
     * @param oRandom the random number stream
     */
    void maximinLHS(int n, int k, int dup, bclib::matrixlhs<int> & result,
            bclib::CRandom<double> & oRandom);
    /**
    ...
    void optimumLHS(int n, int k, int maxSweeps, double eps,
            bclib::matrixlhs<int> & outlhs, int optimalityRecordLength,
    ...
    void optSeededLHS(int n, int k, int maxSweeps, double eps,
            bclib::matrixlhs<double> & pOld, int JLen, bool bVerbose);

    /**
     * type of size type for use with bclib::matrixlhs<T>
     * @note the type of the matrix (i.e. int) is irrelevant for size_type
     */
    typedef bclib::matrixlhs<int>::size_type msize_type;
    /**
     ...
     * @param oRandom the random number stream
     */
    void randomLHS(int n, int k, bool bPreserveDraw, bclib::matrixlhs<double> & result, bclib::CRandom<double> & oRandom);
     ...
     * @param oRandom the random number stream
     */
    void randomLHS(int n, int k, bclib::matrixlhs<int> & result, bclib::CRandom<double> & oRandom);
    ...
    void geneticLHS(int n, int k, int pop, int gen, double pMut, std::string criterium,
            bool bVerbose, bclib::matrixlhs<double> & result, bclib::CRandom<double> & oRandom);
}
