...
namespace lhslib
{
     ...
     */
	bool isValidLHS(const bclib::matrixlhs<int> & result);
    /**
     * is the Latin hypercube sample valid?
     * @param result the latin hypercube sample
     * @return the result of the validity check
     */
    bool isValidLHS(const bclib::matrixlhs<double> & result);
     ...
     */
    void initializeAvailableMatrix(bclib::matrixlhs<int> & avail);
     ...
     */
	template <class T>
	void lhsPrint(const bclib::matrixlhs<T> & A)
	{
        PRINT_MACRO << "\n";
    ...
    T calculateDistanceSquared(const typename bclib::matrixlhsConstIter<T,ISROWWISE> Abegin, 
            const typename bclib::matrixlhsConstIter<T,ISROWWISE> Aend, 
            const typename bclib::matrixlhsConstIter<T,ISROWWISE> Bbegin)
    {
        // sum = sum + (a-b)*(a-b)
     ...
     */
    template <class T>
    void calculateDistance(const bclib::matrixlhs<T> & mat, bclib::matrixlhs<double> & result)
    {
        msize_type m_rows = mat.rowsize();
        if (result.rowsize() != m_rows || result.colsize() != m_rows)
        {
            result = bclib::matrixlhs<double>(m_rows, m_rows);
        }
        for (msize_type i = 0; i < m_rows - 1; i++)
        {
            for (msize_type j = i+1; j < m_rows; j++)
            {
                typename bclib::matrixlhs<T>::const_rowwise_iterator rowi_begin = mat.rowwisebegin(i);
                typename bclib::matrixlhs<T>::const_rowwise_iterator rowi_end = mat.rowwiseend(i);
                typename bclib::matrixlhs<T>::const_rowwise_iterator rowj_begin = mat.rowwisebegin(j);
     ...
     */
 	template <class T>
	double sumInvDistance(const bclib::matrixlhs<T> & A)
    {
        // create a matrix to hold the distances
        bclib::matrixlhs<double> dist = bclib::matrixlhs<double>(A.rowsize(), A.rowsize());
        // calculate the distances between the rows of A
        calculateDistance<T>(A, dist);
        // invert all the distances
        std::transform<bclib::matrixlhs<double>::iterator>(dist.begin(), dist.end(), 
                dist.begin(), invert<double, double>());
        // sum the inverted 
        double totalInvDistance = std::accumulate<bclib::matrixlhs<double>::iterator>(dist.begin(), dist.end(), 0.0);
     ...
     */
	template <class T>
	double sumInvDistance_deprecated(const bclib::matrixlhs<T> & A)
	{
     ...
     */
    template <class T>
    void copyMatrix(bclib::matrixlhs<T> & copyTo, const bclib::matrixlhs<T> & copyFrom)
    {
        ...
        std::copy<typename bclib::matrixlhs<T>::const_iterator, typename bclib::matrixlhs<T>::iterator>(copyFrom.begin(), copyFrom.end(), copyTo.begin());
    }
     ...
     */
    template <class T>
    double calculateSOptimal(const bclib::matrixlhs<T> & mat)
    {
