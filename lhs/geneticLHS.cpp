...
namespace lhslib 
{
            ...
            bool bVerbose, bclib::matrixlhs<double> & result, bclib::CRandom<double> & oRandom)
    {
        ...
        std::vector<bclib::matrixlhs<int> > A = std::vector<bclib::matrixlhs<int> >(m_pop);
        for (msize_type i = 0; i < m_pop; i++)
        {
            A[i] = bclib::matrixlhs<int>(m_n, m_k);
        }
        ...
        std::vector<double> B;
        std::vector<bclib::matrixlhs<int> > J;
        bclib::matrixlhs<double> dist;
        ...
        for (msize_type v = 0; v < m_gen; v++)
        {
            ...
            for (msize_type i = 0; i < m_pop; i++)
            {
                ...
                {
                    //B[i] <- min(dist(A[, , i]))
                    dist = bclib::matrixlhs<double>(A[i].rowsize(), A[i].rowsize());
                    calculateDistance<int>(A[i], dist);
                    // we want to find the minimum distance element, but there are zeros in the dist matrix
                    distnonzero.clear();
                    for (bclib::matrixlhs<double>::const_iterator mit = dist.begin(); mit != dist.end(); ++mit)
                    {
                        if (*mit > 0.0)
                        {
            ...
            J = std::vector<bclib::matrixlhs<int> >(m_pop);
            for (std::vector<bclib::matrixlhs<int> >::iterator i = J.begin(); i != J.end(); ++i)
            {
                *i = bclib::matrixlhs<int>(m_n, m_k);
            }
#ifdef _DEBUG
